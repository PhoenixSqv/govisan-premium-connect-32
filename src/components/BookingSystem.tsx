  ...
  return <>
      ...
      <Dialog open={isOpen} onOpenChange={setIsOpen}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          {step === 'type' && <div className="space-y-6">
              <h3 className="text-lg font-semibold">Selecciona el tipo de consulta</h3>
              <div className="grid gap-4">
                {bookingTypes.map(type => <Card key={type.id} className={`p-4 cursor-pointer border-2 transition-all hover:shadow-lg ${selectedType?.id === type.id ? 'border-govisan-gold bg-govisan-gold/5' : 'border-gray-200 hover:border-govisan-gold/50'}`} onClick={() => setSelectedType(type)}>
              ...
              <Button onClick={() => setStep('datetime')} disabled={!selectedType} className="w-full bg-govisan-gold hover:bg-govisan-gold/90">
                Continuar
              </Button>
            </div>}
          ...
          {step === 'datetime' && <div className="space-y-6">
              <div>
                ...
                <div className="mb-6">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {availableDates.map(date => <Button key={date.value} variant={selectedDate === date.value ? 'default' : 'outline'} onClick={() => setSelectedDate(date.value)} className="text-left h-auto p-3">
                        <div>
                ...
                {selectedDate && <div>
                    ...
                    <div className="grid grid-cols-4 gap-2">
                      {timeSlots.map(slot => <Button key={slot.time} variant={selectedTime === slot.time ? 'default' : 'outline'} disabled={!slot.available} onClick={() => setSelectedTime(slot.time)} size="sm">
                          {slot.time}
                        </Button>)}
                    </div>
                  </div>}
              ...
              <div className="flex gap-3">
                ...
                <Button onClick={() => setStep('details')} disabled={!selectedDate || !selectedTime} className="flex-1 bg-govisan-gold hover:bg-govisan-gold/90">
                  Continuar
                </Button>
              </div>
            </div>}
          ...
          {step === 'details' && <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="text" value={bookingDetails.name} onChange={e => setBookingDetails(prev => ({
                ...prev,
                name: e.target.value
              }))} className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-govisan-gold" required />
                </div>
                <div>
                  <label className="block text-sm font-medium mb-2">Email *</label>
                  <input type="email" value={bookingDetails.email} onChange={e => setBookingDetails(prev => ({
                ...prev,
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="text" value={bookingDetails.company} onChange={e => setBookingDetails(prev => ({
                ...prev,
                ...
                <div>
                  ...
                  <input type="tel" value={bookingDetails.phone} onChange={e => setBookingDetails(prev => ({
                ...prev,
              ...
              <div>
                ...
                <select value={bookingDetails.projectType} onChange={e => setBookingDetails(prev => ({
              ...prev,
              ...
              <div>
                ...
                <textarea value={bookingDetails.message} onChange={e => setBookingDetails(prev => ({
              ...prev,
              message: e.target.value
            }))} rows={3} className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-govisan-gold" placeholder="Cuéntanos más sobre tu proyecto..." />
              </div>
              ...
              <div className="flex gap-3">
                ...
                <Button onClick={handleBooking} disabled={!bookingDetails.name || !bookingDetails.email} className="flex-1 bg-govisan-gold hover:bg-govisan-gold/90">
                  Confirmar Cita
                </Button>
              </div>
            </div>}

          {step === 'confirmation' && <div className="text-center space-y-6">
              ...
              setIsOpen(false);
                ...
                <Button onClick={() => window.open('https://calendar.google.com', '_blank')} className="flex-1 bg-govisan-gold hover:bg-govisan-gold/90">
                  Añadir a Calendario
                </Button>
              </div>
            </div>}
